...
export const verifyOtp = (email, code) => api.post('/accounts/verify-otp/', { email, code });
export const resendOtp = (email) => api.post('/accounts/resend-otp/', { email });
...
export const respondDraw = (gameId, decision) => api.post(`/games/${gameId}/respond-draw/`, { decision });
...
export const fetchGameAnalysisStatus = (gameId) => api.get(`/games/${gameId}/analysis/request/`);
export const requestGameAnalysis = (gameId, payload = {}) => api.post(`/games/${gameId}/analysis/full/`, payload);
export const createPrediction = (gameId, predicted_result) => api.post(`/games/${gameId}/predict/`, { predicted_result });
export const fetchGameEvents = (gameId, since) => api.get(`/games/${gameId}/events/${since ? `?since=${since}` : ''}`);
...
export const registerTournament = (tournamentId, payload = {}) => api.post(`/games/tournaments/${tournamentId}/register/`, payload);
export const unregisterTournament = (tournamentId) => api.post(`/games/tournaments/${tournamentId}/unregister/`);
export const tournamentStandings = (tournamentId) => api.get(`/games/tournaments/${tournamentId}/standings/`);
export const tournamentPairings = (tournamentId) => api.get(`/games/tournaments/${tournamentId}/pairings/`);
export const tournamentMyGame = (tournamentId) => api.get(`/games/tournaments/${tournamentId}/my-game/`);

export const enqueueMatchmaking = (time_control) => api.post('/games/matchmaking/enqueue/', { time_control });
export const cancelMatchmaking = (time_control) => api.post('/games/matchmaking/cancel/', time_control ? { time_control } : {});
export const queueStatus = () => api.get('/games/matchmaking/status/');
export const listBots = (mode = 'blitz') => api.get(`/games/bots/?mode=${mode}`);
export const createBotGame = (botId, payload = {}) => api.post('/games/bots/create-game/', { bot_id: botId, ...payload });
...
export const fetchPublicAccount = (username) => api.get(`/public/accounts/${username}/`);
...
/** Send friend request by user id (number) or payload { to_user_id } */
export const sendFriendRequest = (payload) => {
  if (payload && typeof payload === 'object' && payload.to_user_id != null) {
...
export const getFriendRequests = () => api.get('/social/friend-requests/');
export const respondFriendRequest = (requestId, decision) => api.post(`/social/friend-requests/${requestId}/respond/`, { decision });
export const getFriends = () => api.get('/social/friends/');

export const createThread = (participantId) => api.post('/social/chat/threads/', { participant_id: participantId });
export const listThreads = () => api.get('/social/chat/threads/');
export const getMessages = (threadId) => api.get(`/social/chat/threads/${threadId}/messages/`);
export const sendMessage = (threadId, payload) => api.post(`/social/chat/threads/${threadId}/messages/`, payload);

export const fetchNotifications = (params = {}) => {
  const query = new URLSearchParams(params).toString();
  return api.get(`/notifications/${query ? `?${query}` : ''}`);
};
export const fetchUnreadNotifications = () => api.get('/notifications/unread-count/');
export const markNotificationsRead = (notificationId) => api.post(`/notifications/${notificationId}/mark-read/`);
export const markAllNotificationsRead = () => api.post('/notifications/mark-read/');
export const deleteNotification = (notificationId) => api.del(`/notifications/${notificationId}/`);
