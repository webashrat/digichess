...
import { fetchClock, fetchGameEvents, getGame, spectateGame } from '@/lib/api';
...
function buildWsUrl(path, token) {
  ...
  if (base) return `${base}${path}${token ? `?token=${token}` : ''}`;
  const protocol = typeof window !== 'undefined' && window.location.protocol === 'https:' ? 'wss' : 'ws';
  const host = typeof window !== 'undefined' ? window.location.host : 'localhost:3000';
  return `${protocol}://${host}${path}${token ? `?token=${token}` : ''}`;
}

function deriveTurnFromFen(fen) {
  if (!fen) return null;
...
export default function useGameSync({ gameId, spectate = false, token }) {
  ...
  const mergeState = (payload) => {
    ...
    setState((prev) => {
      const nextLegalMoves = payload.legal_moves ?? payload.game_state?.legal_moves?.san ?? prev?.legal_moves;
      const nextLegalMovesUci = payload.legal_moves_uci ?? payload.game_state?.legal_moves?.uci ?? prev?.legal_moves_uci;
      const nextDrawOffer = hasDrawOffer ? payload.draw_offer_by : (hasGameDrawOffer ? payload.game.draw_offer_by : prev?.draw_offer_by);
      ...
      const nextTurn = payload.turn ?? deriveTurnFromFen(payload.fen ?? payload.game?.current_fen) ?? prev?.turn;
      const nextLastMove = payload.last_move_at ?? prev?.last_move_at;
      const nextServerTime = payload.server_time ?? prev?.server_time;
      const nextRematchBy = hasRematchBy ? payload.rematch_requested_by : (hasGameRematchBy ? payload.game?.rematch_requested_by : prev?.rematch_requested_by);
      const nextRematchAt = hasRematchAt ? payload.rematch_requested_at : (hasGameRematchAt ? payload.game?.rematch_requested_at : prev?.rematch_requested_at);
      ...
      if (prev && nextFen === prev.fen && nextMoves === prev.moves && nextStatus === prev.status && nextResult === prev.result && nextDeadline === prev.first_move_deadline && nextColor === prev.first_move_color && nextDrawOffer === prev.draw_offer_by && nextWhite === prev.white_time_left && nextBlack === prev.black_time_left && nextTurn === prev.turn && nextLastMove === prev.last_move_at && nextServerTime === prev.server_time && nextRematchBy === prev.rematch_requested_by && nextRematchAt === prev.rematch_requested_at && nextRematchStatus === prev.rematch_status && nextRematchGameId === prev.rematch_game_id) {
        return prev;
      }
      return {
        ...prev,
  ...
  const handleEvent = useCallback((payload) => {
    ...
    if (payload.type === 'gameFull') {
      ...
      setState((prev) => ({ ...prev, ...payload.game, fen: payload.game?.current_fen, moves: payload.game?.moves, legal_moves: payload.game?.legal_moves ?? payload.game?.game_state?.legal_moves?.san, game_state: payload.game?.game_state }));
    ...
    if (payload.type === 'chat') {
      ...
      if (recentChatsRef.current.some((entry) => entry.key === key)) return;
    ...
    if (payload.type && payload.type.startsWith('rematch_')) {
      ...
      const nextGameId = payload.type === 'rematch_accepted' ? (payload.rematch_game_id ?? payload.game_id) : undefined;
      const hasRematchBy = Object.prototype.hasOwnProperty.call(payload, 'rematch_requested_by');
      const hasRematchAt = Object.prototype.hasOwnProperty.call(payload, 'rematch_requested_at');
      setState((prev) => ({ ...prev, rematch_requested_by: hasRematchBy ? payload.rematch_requested_by : prev?.rematch_requested_by, rematch_requested_at: hasRematchAt ? payload.rematch_requested_at : prev?.rematch_requested_at, rematch_status: nextStatus, rematch_game_id: nextGameId ?? prev?.rematch_game_id }));
      if (payload.game) setGame((prev) => ({ ...prev, ...payload.game }));
      return;
    }
    if (payload.type === 'clock') {
      mergeState({ white_time_left: payload.white_time_left, black_time_left: payload.black_time_left, turn: payload.turn, server_time: payload.server_time ?? Math.floor(Date.now() / 1000) });
      return;
    }
    if (payload.type === 'game_finished') {
      if (payload.seq) {
        if (payload.seq > lastSeqRef.current + 1 && syncRef.current) syncRef.current();
  ...
  const refreshSnapshot = useCallback(async () => {
    ...
    setGame(data);
    setState((prev) => ({ ...prev, ...data, fen: data.current_fen, moves: data.moves, legal_moves: data.legal_moves, legal_moves_uci: data.legal_moves_uci }));
  }, [gameId, spectate]);
  ...
  const syncEvents = useCallback(async (force = false) => {
    ...
    try {
      ...
      if (data.last_seq) updateSeq(data.last_seq);
      setError(null);
    } catch (err) {
      if (err?.status === 404) eventsUnsupportedRef.current = true;
      try {
        await refreshSnapshot();
        if (activeGameIdRef.current !== targetGameId) return;
        setError(null);
  ...
  useEffect(() => { syncRef.current = syncEvents; }, [syncEvents]);
  ...
  useEffect(() => {
    ...
    const loadGame = async () => {
      ...
      try {
        ...
        if (activeGameIdRef.current !== targetGameId) setError(err.message || 'Failed to load game.');
      }
    };
    loadGame();
  }, [gameId, spectate]);
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      wsRef.current = ws;
      ws.onopen = () => {
        if (activeGameIdRef.current !== targetGameId) { ws.close(); return; }
        setConnected(true);
        setError(null);
      };
      ws.onmessage = (event) => {
  ...
  useEffect(() => {
    ...
    pollingRef.current = setInterval(() => syncEvents(), 3000);
  ...
  const sendChat = (message, room = 'players') => {
    ...
    if (room && room !== 'players') payload.room = room;
    wsRef.current.send(JSON.stringify(payload));
  };

  return { connected, game, state, chat, lastSeq, error, sendChat, syncEvents };
}
